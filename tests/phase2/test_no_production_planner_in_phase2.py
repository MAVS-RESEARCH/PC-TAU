...
def test_no_production_planner_in_phase2():
    ...
    log = subprocess.run(
        ["git", "log", "--oneline", "<AUTHOR_REPO_COMMIT_010>", "--", "src/pc_tau/planner.py", "src/pc_tau/freeze.py"],
        cwd=str(REPO_ROOT),
        capture_output=True,
        text=True,
    )
    ...
    # REVIEW-SNAPSHOT: canonical history intentionally omitted (orphan branch).
    # Original Phase-2 seal commit redacted to <AUTHOR_REPO_COMMIT_010>.
    # History check skipped when commit absent; file-content gate below still enforced.
    if log.returncode != 0:
        print("[review] history omitted (orphan snapshot); skipping history assertion, enforcing file-content gate only.")
    else:
